...
export default function hamburgerMenu() {
    ...
    $(e.currentTarget).toggleClass(STATE_CLASS.ACTIVE);
    $(e.currentTarget).children().toggleClass(STATE_CLASS.ACTIVE);
    $html.toggleClass(STATE_CLASS.DISABLE);

    if($(e.currentTarget).hasClass(STATE_CLASS.ACTIVE)) {
    	scrollLock();
    } else {
    	scrollAble();
    }
	...
	$window.resize(() => {
		if($window.width() > 767) {
			...
			$navButton.removeClass(STATE_CLASS.ACTIVE);
			$navButton.children().removeClass(STATE_CLASS.ACTIVE);
