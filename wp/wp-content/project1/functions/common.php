...
**/
function string_limiter($input, $length) {
  $result = '';
...
function redirect_author() {
    if(is_author()) {
      ...
      if(get_field('hide_author_on_articles_and_author_page', 'user_'. $curauth->ID)) {
        return wp_redirect("404 Not Found");
        exit();
      }
    }
}
add_action( 'wp', 'redirect_author' );
