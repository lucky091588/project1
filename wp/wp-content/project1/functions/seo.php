...
add_filter('wpseo_title', 'fix_author_wpseo_title');
function fix_author_wpseo_title($title) {
    if(  is_author() ) {
        $user_id = get_queried_object_id();
        $title   = get_user_meta($user_id, 'last_name', true) .' '. get_user_meta($user_id, 'first_name', true) .'の記事一覧 | ZOOREL';
    }
    return $title;
}
